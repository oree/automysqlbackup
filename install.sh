...
precheck_files=( automysqlbackup 20754688ca56081610eb16796d768f20
automysqlbackup.conf 951530232c47b52a631622ce3580ae77
README eedddee0f480ce9647f5053ac1b9497c
...
echo "### Creating global configuration directory /etc/automysqlbackup:"
echo
if [ -d /etc/automysqlbackup ]; then
  echo "exists already ... searching for config files:"
  for i in /etc/automysqlbackup/*.conf; do
    [[ "x$(basename $i)" = "xautomysqlbackup.conf" ]] && continue
    parse_config_file "$i"
  done
else
  if mkdir /etc/automysqlbackup >/dev/null 2>&1; then
    #testing for permissions
    if [ -r /etc/automysqlbackup -a -x /etc/automysqlbackup ]; then
      printf "success\n"
    else
      printf "directory successfully created but has wrong permissions, trying to correct ... "
      if chmod +rx /etc/automysqlbackup >/dev/null 2>&1; then
	printf "corrected\n"
      else
...
echo "### Copying files."
echo
cp -i automysqlbackup.conf LICENSE README /etc/automysqlbackup/
cp -i automysqlbackup.conf /etc/automysqlbackup/myserver.conf
cp -i automysqlbackup /usr/local/bin
[[ -f /usr/local/bin/automysqlbackup ]] && [[ -x /usr/local/bin/automysqlbackup ]] || chmod +x /usr/local/bin/automysqlbackup || echo " failed - make sure you make the program executable, i.e. run 'chmod +x /usr/local/bin/automysqlbackup'"
echo
...
if echo $PATH | grep /usr/local/bin >/dev/null 2>&1; then
  ...
  printf "if not, you have to check if 'echo \$PATH' has /usr/local/bin in it\n"
  printf "\nSetup Complete!\n"
else
  printf "if running under the current user, you have to use the full path /usr/local/bin/automysqlbackup since /usr/local/bin is not in 'echo \$PATH'\n"
  printf "\nSetup Complete!\n"
fi
